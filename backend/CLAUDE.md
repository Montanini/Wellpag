...
- `jjwt` 0.12.5 e `springdoc-openapi-starter-webmvc-ui` são pinados explicitamente no `pom.xml` de cada módulo — não são gerenciados pelo BOM do `spring-boot-starter-parent`, então não sobem sozinhos quando o Spring Boot é atualizado. Se atualizar a versão em um módulo, considere se os outros 7 também precisam.
